/*
    gamlang compiler v4 by falkman
    Last revision: October 27, 2009
    ...
    Changes in this version:
        Changed some functions to void, which did not have a return value other than 0.
        Added an error checker for if a gamlang function returned less than 0.
        String variables, along with ability to overwrite them!
        Bug fixes galore, too many to list.
*/
...
int addvar(char identifier[50], char *string, int length, char type)
{
    var_t *varscan;
    ...
    if(!varscan){
        ...
        if(length > varscan->len){
            puts("Error: Variable is too long for previous variable");
            return -1;
        }
        strcpy(prog.data + varscan->dataoff, string);
        varscan->len = length;
    }
...
int storestr(char *code, func_t *func)
{
    ...
    *ptr = 0;
    ...
    addvar(identifier, string, stringlen, 's');
